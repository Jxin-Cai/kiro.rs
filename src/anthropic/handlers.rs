...
use crate::kiro::provider::RequestAuthContext;
...
pub async fn post_messages(
    ...
    if websearch::has_web_search_tool(&payload) {
        ...
        return websearch::handle_websearch_request(provider, &payload, input_tokens, auth_context)
            .await;
    }
...
async fn handle_stream_request(
    ...
    let response = match provider
        .call_api_stream_with_auth(request_body, auth_context)
        .await
    {
        Ok(resp) => resp,
...
async fn handle_non_stream_request(
    ...
    // 调用 Kiro API（支持多凭据故障转移）
    let response = match provider
        .call_api_with_auth(request_body, auth_context)
        .await
    {
        Ok(resp) => resp,
        Err(e) => return map_provider_error(e),
